...
| 3       | **用户主动登出(单点退出)**       | 用户在当前设备上点击“退出登录”，清除本地 Token/加入黑名单，同时服务端标记该 Token 为无效（如加入黑名单）。      |
| v2      | **全局登出（退出所有设备）**     | 用户触发“退出所有设备”，清除所有设备上的 Token（可通过清除 Redis 记录实现），常用于账号被盗或安全策略要求。     |
| v2      | **密码修改**                     | 用户已知当前密码，主动修改为新密码，常用于安全习惯或防止他人登录                                                  |
| 6/v2    | **密码重置验证（忘记密码）**     | 用户忘记密码，需通过手机/邮箱验证码验证身份后设置新密码。用于恢复账号访问权限。                                   |
| v2      | **绑定手机 / 更换手机号**        | 用于增强账号安全性、接收验证码、辅助找回密码等功能；更换手机号时需验证旧手机号或登录密码。                        |
| v2      | **绑定邮箱 / 更换邮箱**          | 提高账号可恢复性和安全性，邮箱也可用于接收验证码、验证身份、通知提醒等。                                          |
...
失败的设计：登录用的是用户名与密码，但是谁会记用户名，v2后续一定要先做 邮箱与密码登录  （扩展：用手机短信注册，多邮箱呢？）

一个邮箱多个账号，邮箱登录，给出对应的用户列表 （后面）
